...
- in_scope: true if the question and the answer both stay within FounderLex's educational scope (plain-English startup legal basics: business structure, intellectual property, contracts, liability, compliance, consulting, and nonprofit topics). False if the underlying topic is actually a lawsuit or active dispute, securities/fundraising terms, immigration, a criminal matter, or tax strategy — those should have been refused and referred to a licensed professional instead of substantively answered.
- supported_by_sources: true if every specific factual claim in the answer (a fee, a deadline, a statutory citation, a percentage or dollar threshold) is consistent with the reference material provided, or if the answer makes no such specific claims. False if it states a specific fact that is not present in, or contradicts, the reference material, or invents a number. If no reference material was provided, judge this more leniently — only flag false if the answer states a suspiciously specific fact with no clear general-knowledge basis.
...
Be strict. If you are genuinely uncertain whether a claim is supported or whether the topic is in scope, flag it rather than letting it through.`
